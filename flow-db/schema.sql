...
CREATE TABLE IF NOT EXISTS documents (
  ...
  CHECK (has_context = 0 OR external_doc_id IS NOT NULL)
);
...
CREATE INDEX IF NOT EXISTS idx_lex_var_entry ON lexicon_variants(lexicon_entry_id);
...
CREATE INDEX IF NOT EXISTS idx_error_cases_source ON error_cases(source_id);
CREATE INDEX IF NOT EXISTS idx_error_cases_variant_type ON error_cases(variant_type);
